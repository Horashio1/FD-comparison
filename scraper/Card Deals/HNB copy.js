    ...
    try {
        ...
        const details = await page.evaluate(() => {
            ...
            fields.offer = (() => {
                ...
                return offerText.trim() || 'No offer';
            })();
            
            // Other fields continue using the original extractField function
            fields.period = extractField(/^Period[s]?[:\s]/i, 'No period');
            fields.eligibility = extractField(/^Eligibility[:\s]/i, 'No eligibility');
            fields.contacts = (() => {
                const contactLabels = [
                    /^(Contact[s]?[:]?|Contact No[:]?|Reservation[s]?[:]?|Tel[:]?|Telephone[:]?)/i
                ];
                ...
                return extractTextAfterLabel(contactLabels[0], stopLabels) || '';
            })();

            // Function to extract text after a label, limited to immediate siblings and first-level children
            function extractTextAfterLabel(labelRegex, stopLabelRegexArray = []) {
                const labelElement = Array.from(detailsDiv.querySelectorAll('div, p, span, strong, b')).find(el => {
                    return labelRegex.test(el.textContent.trim());
                });

                if (labelElement) {
                    const collectedTextsSet = new Set();
                    let sibling = labelElement.nextElementSibling;

                    // Check if the content is within the same element
                    let [_, contentAfterLabel] = labelElement.textContent.split(/[:：]\s*/);
                    if (contentAfterLabel && !stopLabelRegexArray.some(regex => regex.test(contentAfterLabel))) {
                        collectedTextsSet.add(contentAfterLabel.trim());
                    }

                    while (sibling) {
                        const siblingText = sibling.textContent ? sibling.textContent.trim() : '';

                        // Stop if we reach a stop label
                        if (stopLabelRegexArray.some(regex => regex.test(siblingText))) {
                            break;
                        }

                        // Collect text from list items or direct text content
                        if (sibling.tagName.toLowerCase() === 'ul' || sibling.tagName.toLowerCase() === 'ol') {
                            const listItems = Array.from(sibling.querySelectorAll('li')).map(li =>
                                li.innerText.trim()
                            );
                            listItems.forEach(item => collectedTextsSet.add(item));
                        } else if (
                            ...
                            if (
                                !labelRegex.test(siblingText) &&
                                !stopLabelRegexArray.some(regex => regex.test(siblingText))
                            ) {
                                collectedTextsSet.add(siblingText);
                            }
                        }

                        // Only consider immediate siblings and first-level children
                        sibling = sibling.nextElementSibling;
                    }

                    return Array.from(collectedTextsSet).join('\n').trim();
                }
                return '';
            }

            // Extract locations
            fields.locations = extractTextAfterLabel(
                /^(Location[s]?[:]?)/i,
                [/^Contact/i, /^Special Terms/i, /^General Terms/i, /^Eligibility/i, /^Offer/i, /^Period/i]
            ) || '';
